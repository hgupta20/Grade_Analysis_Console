...
public:
  ...
  {
      cout << Name << ":" << endl;
      cout << " # courses taught: " << Courses.size() << endl;
      int numStudents =0;
      for(Course i : Courses)
        {
          numStudents+=i.getNumStudents(); 
        }
      cout << " # students taught: " << numStudents << endl;
  
  }
  vector<Course> findUnknown()
    {
      vector<Course> coursesVec;
      for(Course i : Courses)
        {
          if(i.getGradingType() == Course::Unknown)
            {
              coursesVec.push_back(i);
            }
        }
        sort(coursesVec.begin(),coursesVec.end(),
  [](Course s1, Course s2)
    {
...
public:
  ...
  College(string name)
   : Name(name)
  { }
  // A method to findUnknown courses which can be accessed by college
    vector<Course> findUnknown()
    {
      vector<Course> coursesVec;
      for(Dept i : Depts)
        {
          ...
          if(j.getGradingType() == Course::Unknown)
            {
              coursesVec.push_back(j);
            }
        }
        sort(coursesVec.begin(),coursesVec.end(),
  [](Course s1, Course s2)
